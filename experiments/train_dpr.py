...
def train_model(args, device):
    ...
    # Save the model
    print('Saving model..')
    dpr_model.question_encoder.question_encoder.bert_model.save_pretrained(args.save_dir + args.model + str(args.max_seq_length) + "_" + str(args.embeddings_size) + '/question_encoder/')
    dpr_model.context_encoder.ctx_encoder.bert_model.save_pretrained(args.save_dir + args.model + str(args.max_seq_length) + "_" + str(args.embeddings_size) + '/context_encoder/')
    if args.embeddings_size > 0:
        torch.save(dpr_model.question_encoder.question_encoder.encode_proj.state_dict(), args.save_dir + args.model + str(args.max_seq_length) + "_" + str(args.embeddings_size) + '/question_encoder_projection.pth')
        torch.save(dpr_model.context_encoder.ctx_encoder.encode_proj.state_dict(), args.save_dir + args.model + str(args.max_seq_length) + "_" + str(args.embeddings_size) + '/context_encoder_projection.pth')
    print('Model saved')
